...
public abstract class BufferBuilderMixin implements VertexBufferWriter, BufferBuilderExtension, VertexConsumerIntrinsicsExtension {
    @Shadow
    ...
    public boolean sodium$canAccept(VertexFormat format) {
        ...
        for (var element : this.format.getElements()) {
            if (!format.contains(element.name())) {
                return false;
            }
        }
