...
public class VertexSerializerFactory {
    ...
    private static List<MemoryTransfer> createMemoryTransferList(VertexFormat srcVertexFormat, VertexFormat dstVertexFormat) {
        ...
        for (var dstElement : dstVertexFormat.getElements()) {
            var elementName = dstElement.name();

            if (!srcVertexFormat.contains(elementName)) {
                throw new RuntimeException("Source format is missing element %s as required by destination format".formatted(elementName));
            }

            var srcElement = srcVertexFormat.getElement(elementName);
            ops.add(new MemoryTransfer(srcElement.offset(), dstElement.offset(), dstElement.format().blockSize()));
        }
